...
import { ErrorsInboxModel } from './fetch-errors-inbox-data';
import { fetchErrorsInboxData } from './fetch-errors-inbox-data'
...
export class ErrorsInboxTag extends React.Component {
  ...
  constructor(props) {
    ...
    this.createTableData = this.props.createTableData || createErrorsInboxTableData;
    this.computeMaturityScore = this.props.computeMaturityScore || computeErrorsInboxMaturityScore;

    this.scoreWeights = this.props.scoreWeights || ErrorsInboxModel.scoreWeights;
  ...
  render() {
    ...
    return (
      ...
      >
        {({ filteredData }) => {
          return <ErrorsInboxTable data={filteredData} columns={this.tableColHeader} />
        }}
      </FilterTableData>
    );
  }
